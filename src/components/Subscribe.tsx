...
const Subscribe = () => {
  ...
  const { content, isVisible } = useContent<SubscribeContent>("subscribe");
    ...
    <div className="container mx-auto px-4">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <h2 className="font-serif text-3xl md:text-4xl font-bold text-foreground mb-4">
          {content?.heading || "Не пропустите новые рецензии и обзоры"}
        </h2>
        <p className="text-muted-foreground text-lg mb-8">
          {content?.description || "Подпишитесь на рассылку..."}
        </p>

        {isSubscribed ? <div className="bg-primary/10 text-primary p-6 rounded-2xl inline-flex items-center gap-3">
          <Sparkles className="w-6 h-6" />
          <span className="font-medium">
            {content?.success_message || "Спасибо за подписку! Скоро напишем."}
          </span>
          ...
          <button type="submit" className="px-8 py-4 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-all hover-lift shadow-warm whitespace-nowrap">
            {content?.button_text || "Подписаться"}
          </button>
        </form>}

        <p className="text-sm text-muted-foreground mt-4">
          {content?.footer_text || "Присоединяйтесь к моим читателям"}
        </p>
      </div>
    </div>
  </section>;
